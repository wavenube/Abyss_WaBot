...
const handlerPerfil = async (m, { conn, usedPrefix }) => {
    const datas = global;
    ...
    try {
        ...
        const { name, limit, lastclaim, registered, regTime, age, premiumTime, personajeReclamado } = global.db.data.users[who];
        const username = conn.getName(who);
        const prem = global.prems.includes(who.split`@`[0]);
        const sn = createHash('md5').update(who).digest('hex');
        const personajes = personajeReclamado ? `**Personajes en Posesión:**\n${personajeReclamado}` : 'No has reclamado ningún personaje aún.';
