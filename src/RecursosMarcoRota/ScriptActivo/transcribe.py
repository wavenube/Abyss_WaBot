...
def transcribe_audio(file_path):
    model = whisper.load_model("small") # Hay Diferentes Niveles de procesamiento, cada uno tiene mas parametros que el anterior, aunque usa mas ram y tarda mas tiempo, pero puede ser mas preciso. "tiny" y "base" son las mas pequeñas y usan alrededor de 1GB de RAM. "small" 2GB "medium" 5GB y "large" 10GB, tambien necesita descargar esa cantidad del ejecutor. Mas informacion en la documentacion de openai/whisper en github.
    result = model.transcribe(file_path, language='es', word_timestamps=False) #append_punctuations=False, prepend_punctuations=False)
    return result['text']

if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Uso: python /plugins/transcribe.py <ruta_al_archivo_de_audio>")
        sys.exit(1)
   ...
   #### Windows: Prioridad mínima para evitar que sature el sistema. debes descomentarlo. Requiere pywin32 
   # pid = os.getpid()
